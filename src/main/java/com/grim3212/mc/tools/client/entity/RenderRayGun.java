...
public class RenderRayGun extends Render<EntityRayw> {
	...
	public void doRender(EntityRayw entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		WorldRenderer renderer = tesselator.getWorldRenderer();
		float minU = 0.0F;
		float maxU = 1.0F;
		float minV = 0.0F;
		float maxV = 1.0F;
		...
		renderer.pos((double) (0.0F - 0.5F), (double) (0.0F - 0.25F), 0.0D).tex((double) minU, (double) maxV).normal(1.0f, 1.0f, 1.0f).endVertex();
		renderer.pos((double) (1.0F - 0.5F), (double) (0.0F - 0.25F), 0.0D).tex((double) maxU, (double) maxV).normal(1.0f, 1.0f, 1.0f).endVertex();
		renderer.pos((double) (1.0F - 0.5F), (double) (1.0F - 0.25F), 0.0D).tex((double) maxU, (double) minV).normal(1.0f, 1.0f, 1.0f).endVertex();
		renderer.pos((double) (0.0F - 0.5F), (double) (1.0F - 0.25F), 0.0D).tex((double) minU, (double) minV).normal(1.0f, 1.0f, 1.0f).endVertex();
		tesselator.draw();
