...
public class DesertGenClassTransformer implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] bytes) {
		if ("net.minecraft.world.biome.BiomeGenDesert".equals(transformedName)) {
			GrimLog.info(GrimWorld.partName, "INSIDE DEOBFUSCATED BIOMEGENDESERT TRANSFORMER ABOUT TO PATCH: " + transformedName);
			try {
	...
	private byte[] patch(String name, byte[] bytes, String desc, String iFile, String rname) throws IOException {
		...
		while (methods.hasNext()) {
			...
			if ((m.name.equals("func_180624_a")) && (m.desc.equals(desc))) {
				GrimLog.info(GrimWorld.partName, "Found Method - func_180624_a");
				mn = m;
				break;
			} else if ((m.name.equals("decorate")) && (m.desc.equals(desc))) {
				GrimLog.info(GrimWorld.partName, "Found Method - decorate");
				mn = m;
				break;
			}
		}
		...
		while (replacemethods.hasNext()) {
			...
			if (m.name.equals("func_180624_a")) {
				GrimLog.info(GrimWorld.partName, "Found Method for Replacement - func_180624_a");
				mnr = m;
				break;
			} else if (m.name.equals("decorate")) {
				GrimLog.info(GrimWorld.partName, "Found Method for Replacement - decorate");
				mnr = m;
				break;
			}
		}
		...
		GrimLog.info(GrimWorld.partName, "Patching Complete");
		cn.accept(cw);
