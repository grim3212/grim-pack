			...
			if (event.getTarget().typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (player.isSneaking()) {
					...
					if (iblockstate.getMaterial() != Material.AIR && world.getWorldBorder().contains(blockpos)) {
						...
						drawSelectionBoundingBox(iblockstate.getSelectedBoundingBox(world, blockpos).grow(0.0020000000949949026D).offset(-d0, -d1, -d2), 0.5F, 0.5F, 0.3F, 0.4F);

						List<AxisAlignedBB> collisionBoxes = Lists.<AxisAlignedBB>newArrayList();
						iblockstate.addCollisionBoxToList(world, blockpos, iblockstate.getSelectedBoundingBox(world, blockpos).grow(3.0d), collisionBoxes, player, false);

						for (int i = 0; i < collisionBoxes.size(); i++)
							drawSelectionBoundingBox(collisionBoxes.get(i).grow(0.0020000000949949026D).offset(-d0, -d1, -d2), 0.0F, 0.0F, 0.2F, 0.4F);
					}
