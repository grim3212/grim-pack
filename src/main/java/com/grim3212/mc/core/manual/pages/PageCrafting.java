...
import com.grim3212.mc.core.client.TooltipHelper;
...
public class PageCrafting extends Page {
	...
	private ResourceLocation craftingOverlay = new ResourceLocation(GrimCore.modID, "textures/gui/craftingOverlay.png");
	...
	public void renderRecipe(GuiSubSectionPage gui, List<IRecipe> output) {
		...
		if (isShapeless)
			NBTHelper.setString(outstack, "customTooltip", StatCollector.translateToLocal("grim.manual.shapeless"));
	...
	public void drawOreDictionaryItem(GuiSubSectionPage gui, Object item, int x, int y) {
		if (item instanceof ItemStack) {
			...
			this.renderItemCutWild(gui, NBTHelper.setStringItemStack(((List<ItemStack>) item).get(0), "customTooltip", StatCollector.translateToLocal("grim.manual.oredictionary") + " : " + RecipeHelper.getOreDict((List<ItemStack>) item)), x - 1, y - 1);
		}
	}
